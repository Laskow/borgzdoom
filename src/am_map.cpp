...
//[las]
CVAR (Int, am_playerarrow, 1, CVAR_ARCHIVE); //[las] Show the sprite by default
...
{
	// [las]
	if (am_playerarrow == 1 || am_cheat >= 2 && am_cheat != 4 && am_showthingsprites > 0)
	{
...
void DAutomap::Drawer (int bottom)
{
	...
	if (am_playerarrow == 1 || (am_cheat >= 2 && am_cheat != 4))
		drawThings(); // [las]
